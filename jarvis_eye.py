#!/usr/bin/env python3
"""
Jarvis Eye - Animated desktop companion with video frames
...
FRAMES_DIR = os.path.join(SCRIPT_DIR, "jarvis_frames_transparent")
...
class JarvisEyeApp(NSObject):
    def init(self):
        self = objc.super(JarvisEyeApp, self).init()
        if self is None:
            return None
...
def main():
    print("Starting Jarvis Eye (Video Animation)...")
    print("Press Ctrl+C in terminal to quit.\n")

    app = NSApplication.sharedApplication()
    app.setActivationPolicy_(NSApplicationActivationPolicyAccessory)

    delegate = JarvisEyeApp.alloc().init()
    delegate.createWindow()

    print("\nJarvis Eye is now running!")
    app.run()
